    ...
    int rr_prim)
{
    ...
    if (subscr->ustate != GSM_SIM_U1_UPDATED) {
        LOGP(DMM, LOGL_INFO, "Init MM Connection, but SIM not updated.\n");
        cause = 21;
        goto reject;
    }
    ...
    if (mm->state == GSM48_MM_ST_MM_IDLE) {
        ...
        case GSM48_MM_SST_ATTEMPT_UPDATE:
            ...
            LOGP(DMM, LOGL_INFO, "Init MM Connection, but attempting to update.\n");
            cause = 21;
            goto reject;
        default:
            LOGP(DMM, LOGL_INFO, "Init MM Connection, not in normal state.\n");
            cause = 21;
            goto reject;
        }
    } else
