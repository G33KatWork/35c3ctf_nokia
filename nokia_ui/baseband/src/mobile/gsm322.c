...
static void new_a_state(struct gsm322_plmn *plmn, int state)
{
    ...
    mobile_prim_ntfy_gsm322_status(plmn->ms, GSM322_STATE_MACHINE_PLMN_AUTOMATIC, state);
    plmn->state = state;
}
...
static void new_m_state(struct gsm322_plmn *plmn, int state)
{
    ...
    mobile_prim_ntfy_gsm322_status(plmn->ms, GSM322_STATE_MACHINE_PLMN_MANUAL, state);
    plmn->state = state;
}
...
static void new_c_state(struct gsm322_cellsel *cs, int state)
{
    ...
    mobile_prim_ntfy_gsm322_status(cs->ms, GSM322_STATE_MACHINE_CELL_SELECTION, state);
    cs->state = state;
}
...
static int gsm322_a_go_on_plmn(struct osmocom_ms *ms, struct msgb *msg)
{
    ...
    mobile_prim_ntfy_network_assignment(ms, plmn->mcc, plmn->mnc);
...
static int gsm322_m_display_plmns(struct osmocom_ms *ms, struct msgb *msg)
{
    ...
    case GSM322_EVENT_REG_FAILED:
        //mobile_prim_ntfy_gsm322_event(ms, GSM322_EVENT_REG_FAILED);
        break;
    case GSM322_EVENT_NO_CELL_FOUND:
        //mobile_prim_ntfy_gsm322_event(ms, GSM322_EVENT_NO_CELL_FOUND);
        break;
    case GSM322_EVENT_ROAMING_NA:
        //mobile_prim_ntfy_gsm322_event(ms, GSM322_EVENT_ROAMING_NA);
        break;
    }
    ...
    if (llist_empty(&plmn->sorted_plmn))
        ...
        //mobile_prim_ntfy_cell_search_end(ms, &plmn->sorted_plmn);
    ...
    //FIXME: this is commented out to prevent the manual network
    //searching from connecting to the home network immediately
    /* we must forward this, otherwhise "Any cell selection"
     * will not start automatically.
...
static int gsm322_c_new_plmn(struct osmocom_ms *ms, struct msgb *msg)
{
    ...
    LOGP(DSUM, LOGL_INFO, "Selecting PLMN (mcc=%s mnc=%s)\n",
...
int gsm322_init(struct osmocom_ms *ms)
{
    ...
    /* read BA list */
    ba_filename = talloc_asprintf(ms, "%s/%s/%s.ba", getenv("HOME"), ".osmocom/bb", ms->name);
