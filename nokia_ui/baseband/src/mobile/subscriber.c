    ...
    uint16_t lac, uint32_t tmsi, uint8_t imsi_attached)
{
    ...
    LOGP(DMM, LOGL_INFO, "(ms %s) Inserting test card (IMSI=%s)\n",
        ms->name, subscr->imsi);
...
static int subscr_sim_request(struct osmocom_ms *ms)
{
    ...
    if (!sf->func) {
        ...
        if (subscr->lac > 0x0000 && subscr->lac < 0xfffe) {
            ...
            mobile_prim_ntfy_sim_status(ms, PRIM_SIM_VALID_UNLOCKED, 0);
        } else {
...
static void subscr_sim_query_cb(struct osmocom_ms *ms, struct msgb *msg)
{
    ...
    if (sh->job_type == SIM_JOB_ERROR) {
        ...
        case SIM_CAUSE_PIN1_REQUIRED:
            ...
            mobile_prim_ntfy_sim_status(ms, PRIM_SIM_PIN1_REQUIRED, payload[1]);
            subscr->sim_pin_required = 1;
            break;
        case SIM_CAUSE_PIN1_BLOCKED:
            LOGP(DMM, LOGL_NOTICE, "PIN is blocked\n");
            mobile_prim_ntfy_sim_status(ms, PRIM_SIM_PIN1_BLOCKED, payload[1]);
            subscr->sim_pin_required = 1;
            break;
        case SIM_CAUSE_PUC_BLOCKED:
            LOGP(DMM, LOGL_NOTICE, "PUC is blocked\n");
            mobile_prim_ntfy_sim_status(ms, PRIM_SIM_PUC_BLOCKED, 0);
            subscr->sim_pin_required = 1;
            break;
        default:
            if (sf->func && !sf->mandatory) {
                LOGP(DMM, LOGL_NOTICE, "SIM reading failed, ignoring!\n");
                goto ignore;
            }
    ...
    int8_t mode)
{
    ...
    if (subscr->sim_type != GSM_SIM_TYPE_SOCK
#ifdef SIM_TRUSTZONE
...
int gsm_subscr_socket_sim(struct osmocom_ms *ms)
{
    struct gsm_subscriber *subscr = &ms->subscr;
...
#ifdef SIM_TRUSTZONE
int gsm_subscr_trustzone_sim(struct osmocom_ms *ms)
{
    struct gsm_subscriber *subscr = &ms->subscr;

    trustzonecard_open(ms);
