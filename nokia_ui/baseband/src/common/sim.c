...
static int sim_apdu_send(struct osmocom_ms *ms, uint8_t *data, uint16_t length)
{
    ...
    if (ms->subscr.sim_type == GSM_SIM_TYPE_SOCK) {
        LOGP(DSIM, LOGL_INFO, "Using Socket backend\n");
        socketcard_send_apdu(ms, data, length);
    } 
#ifdef SIM_TRUSTZONE
    else if (ms->subscr.sim_type == GSM_SIM_TYPE_TRUSTZONE) {
        LOGP(DSIM, LOGL_INFO, "Using Trustzone backend\n");
        trustzonecard_send_apdu(ms, data, length);
    }
#endif
    else {
        LOGP(DSIM, LOGL_ERROR, "Unknown SIM type\n");
    }
